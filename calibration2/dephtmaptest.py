...
while True:
    # Capture grayscale images from the cameras
    imgLeft = dismap.capture_grayscale_image(cap_left)
    imgRight = dismap.capture_grayscale_image(cap_right)
   # ret,imgLeft = cv2.threshold(imgLeft,100,255,0)
   # ret,imgRight = cv2.threshold(imgRight,100,255,0)
    imgRight, imgLeft = calibration.undistortRectify(imgRight, imgLeft)
    # Compute the depth map
    depthmap =dismap.ShowDisparity(imgLeft, imgRight,5)
